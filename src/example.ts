 ...
 * 
 * This file demonstrates both the simple and advanced approaches
 * to generating CIUS-RO compliant invoices.
 */
...
// =============================================================================
// Example 1: Simple Invoice with Automatic Calculations
// =============================================================================
...
function createSimpleInvoice(): string {
  ...
  return invoice.generateXml();
}

// =============================================================================
// Example 2: Advanced Invoice with Builder Pattern
// =============================================================================
...
function createAdvancedInvoice(): string {
  const seller: Seller = {
    ...
    address: {
      ...
      cityName: 'SECTOR1', // Bucharest sector
      postalZone: '010271',
      countrySubentity: 'RO-B',
      countryCode: 'RO',
    },
  ...
  const invoice = new Invoice()
    .setGeneralData({
      ...
      invoicePeriod: {
        ...
        descriptionCode: '35', // Actual delivery date
      },
    })
    .setSeller(seller)
    .setBuyer(buyer)
    .setPaymentMeans(paymentMeans)
    .setPaymentTerms({ note: 'Net 30 zile' })
    .addLine({
...
async function main() {
  ...
  // Example 2
  console.log('📄 Example 2: Advanced Invoice (Builder Pattern)');
  console.log('-'.repeat(40));
  ...
  try {
    ...
    fs.writeFileSync(path.join(outputDir, 'simple-invoice.xml'), simpleXml, 'utf-8');
    fs.writeFileSync(path.join(outputDir, 'advanced-invoice.xml'), advancedXml, 'utf-8');
    fs.writeFileSync(path.join(outputDir, 'credit-note.xml'), creditNoteXml, 'utf-8');
    fs.writeFileSync(path.join(outputDir, 'non-vat-payer.xml'), nonVatXml, 'utf-8');
    fs.writeFileSync(path.join(outputDir, 'allowances-charges.xml'), acXml, 'utf-8');
